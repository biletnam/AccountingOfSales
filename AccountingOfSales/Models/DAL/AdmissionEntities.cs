    ...
    public class AdmissionEntities
    {
        public static List<Admission> GetAdmissions(DateTime? filterDateAdmissionFrom, DateTime? filterDateAdmissionTo, int? filterUser, int? filterProvider)
        {
            ...
            if (filterDateAdmissionFrom != null && filterDateAdmissionTo != null)
                admissions = Config.db.Admissions.Where(d => d.AdmissionDate >= filterDateAdmissionFrom).Where(d => d.AdmissionDate <= filterDateAdmissionTo).ToList();
            else if (filterDateAdmissionFrom != null && filterDateAdmissionTo == null)
                admissions = Config.db.Admissions.Where(d => d.AdmissionDate >= filterDateAdmissionFrom).ToList();
            else if (filterDateAdmissionFrom == null && filterDateAdmissionTo != null)
            {
                ...
                admissions = Config.db.Admissions.Where(d => d.AdmissionDate >= last3MonthsDateTo).Where(d => d.AdmissionDate <= filterDateAdmissionTo).ToList();
            }
            else
                admissions = Config.db.Admissions.Where(d => d.AdmissionDate >= last3Months).ToList();
